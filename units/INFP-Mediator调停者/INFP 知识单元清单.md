# INFP-Mediator调停者 — 知识单元清单

> 来源：`raw/books/INFP-Mediator调停者.md`
> 本文件列出该来源拆解产生的所有知识单元。
> 返回：[[知识单元index|知识单元索引]]

## 理论背景

- `shared-theory-history` — [MBTI理论-历史背景.md](../人格共享内容/MBTI理论-历史背景.md)
- `shared-type-groups-overview` — [类型组概述.md](类型组概述.md)

## 五大维度

- `shared-dimension-ie` — [心智维度-内倾型（I）与外倾型（E）.md](../人格共享内容/心智维度-内倾型（I）与外倾型（E）.md)
- `shared-dimension-ns` — [能量维度-直觉型（N）与实感型（S）.md](../人格共享内容/能量维度-直觉型（N）与实感型（S）.md)
- `shared-dimension-tf` — [本性维度-思考型（T）与情感型（F）.md](../人格共享内容/本性维度-思考型（T）与情感型（F）.md)
- `shared-dimension-jp` — [行动维度-计划型（J）与灵活型（P）.md](../人格共享内容/行动维度-计划型（J）与灵活型（P）.md)
- `shared-dimension-at` — [身份维度-自信型（-A）与动荡型（-T）.md](../人格共享内容/身份维度-自信型（-A）与动荡型（-T）.md)

## 四大角色组

- `shared-theory-roles` — [角色概述.md](../人格共享内容/角色概述.md)
- `shared-typegroup-nt` — [类型组-NT分析家.md](../人格共享内容/类型组-NT分析家.md)
- `shared-typegroup-nf` — [类型组-NF外交家.md](../人格共享内容/类型组-NF外交家.md)
- `shared-typegroup-sj` — [类型组-SJ守卫者.md](../人格共享内容/类型组-SJ守卫者.md)
- `shared-typegroup-sp` — [类型组-SP探险家.md](../人格共享内容/类型组-SP探险家.md)

## 四大策略

- `shared-theory-strategies` — [策略概述.md](../人格共享内容/策略概述.md)
- `shared-strategy-confident-individualism` — [策略-自信独行型（内倾型×自信型）.md](../人格共享内容/策略-自信独行型（内倾型×自信型）.md)
- `shared-strategy-people-mastery` — [策略-人际掌控型（外倾型×自信型）.md](../人格共享内容/策略-人际掌控型（外倾型×自信型）.md)
- `shared-strategy-constant-improvement` — [策略-持续精进型（内倾型×动荡型）.md](../人格共享内容/策略-持续精进型（内倾型×动荡型）.md)
- `shared-strategy-social-engagement` — [策略-社交投入型（外倾型×动荡型）.md](../人格共享内容/策略-社交投入型（外倾型×动荡型）.md)

## INFP 类型专属

**academic**
- `INFP-academic-overview` — [INFP/academic-总论.md](INFP/academic-总论.md)
- `INFP-academic-high-school` — [INFP/academic-高中.md](INFP/academic-高中.md)
- `INFP-academic-university` — [INFP/academic-大学.md](INFP/academic-大学.md)
- `INFP-academic-lifelong-learning` — [INFP/academic-终身学习.md](INFP/academic-终身学习.md)

**career**
- `INFP-career-overview` — [INFP/career-总论.md](INFP/career-总论.md)
- `INFP-career-workplace-roles` — [INFP/career-职场角色.md](INFP/career-职场角色.md)

**dimension-trait**
- `INFP-dimension-trait-introversion` — [INFP/dimension-trait-内倾型.md](INFP/dimension-trait-内倾型.md)
- `INFP-dimension-trait-combined` — [INFP/dimension-trait-综合特质.md](INFP/dimension-trait-综合特质.md)

**friendship**
- `INFP-friendship-overview` — [INFP/friendship-总论.md](INFP/friendship-总论.md)

**overview**
- `INFP-overview-core` — [INFP/overview-概述.md](INFP/overview-概述.md)

**parenting**
- `INFP-parenting-overview` — [INFP/parenting-总论.md](INFP/parenting-总论.md)
- `INFP-parenting-development-stages` — [INFP/parenting-发展阶段.md](INFP/parenting-发展阶段.md)

**personal-growth**
- `INFP-growth-overview` — [INFP/personal-growth-总论.md](INFP/personal-growth-总论.md)
- `INFP-growth-self-esteem` — [INFP/personal-growth-自尊.md](INFP/personal-growth-自尊.md)
- `INFP-growth-self-love` — [INFP/personal-growth-自爱.md](INFP/personal-growth-自爱.md)
- `INFP-growth-confidence` — [INFP/personal-growth-自信.md](INFP/personal-growth-自信.md)
- `INFP-growth-self-evolution` — [INFP/personal-growth-自我进化.md](INFP/personal-growth-自我进化.md)
- `INFP-growth-self-responsibility` — [INFP/personal-growth-自我负责.md](INFP/personal-growth-自我负责.md)

**romantic**
- `INFP-romantic-overview` — [INFP/romantic-总论.md](INFP/romantic-总论.md)

## 关系单元

### INFP 恋爱
- `INFP-NT型人-romantic` — [relationships/INFP-NT型人-恋爱.md](relationships/INFP-NT型人-恋爱.md)
- `INFP-NF型人-romantic` — [relationships/INFP-NF型人-恋爱.md](relationships/INFP-NF型人-恋爱.md)
- `INFP-SJ型人-romantic` — [relationships/INFP-SJ型人-恋爱.md](relationships/INFP-SJ型人-恋爱.md)
- `INFP-SP型人-romantic` — [relationships/INFP-SP型人-恋爱.md](relationships/INFP-SP型人-恋爱.md)

### INFP 友谊
- `INFP-NT型人-friendship` — [relationships/INFP-NT型人-友谊.md](relationships/INFP-NT型人-友谊.md)
- `INFP-NF型人-friendship` — [relationships/INFP-NF型人-友谊.md](relationships/INFP-NF型人-友谊.md)
- `INFP-SJ型人-friendship` — [relationships/INFP-SJ型人-友谊.md](relationships/INFP-SJ型人-友谊.md)
- `INFP-SP型人-friendship` — [relationships/INFP-SP型人-友谊.md](relationships/INFP-SP型人-友谊.md)

### INFP 职场协作
- `INFP-NT型人-career` — [relationships/INFP-NT型人-职场协作.md](relationships/INFP-NT型人-职场协作.md)
- `INFP-NF型人-career` — [relationships/INFP-NF型人-职场协作.md](relationships/INFP-NF型人-职场协作.md)
- `INFP-SJ型人-career` — [relationships/INFP-SJ型人-职场协作.md](relationships/INFP-SJ型人-职场协作.md)
- `INFP-SP型人-career` — [relationships/INFP-SP型人-职场协作.md](relationships/INFP-SP型人-职场协作.md)

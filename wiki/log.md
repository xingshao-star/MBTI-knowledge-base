...
## [2026-05-05] compile | 编译"人格共享内容"知识单元到Wiki

- **来源**：`units/人格共享内容/`（16个共享知识单元）
- **操作**：用完整版知识单元正文替换Wiki中已有的精简版页面，并新建2个理论页面
- **替换页面**（14个）：
  - theory/：MBTI理论-历史背景（补充五大维度概览、人格理论局限性等段落）
  - dimensions/：心智维度、能量维度、本性维度、行动维度、身份维度（补充各类型具体表现、地域分布数据、蓄能/参与力量分析等）
  - type-groups/：NT-分析家、NF-外交家、SJ-守卫者、SP-探险家（补充天赋才能、研究数据、全球分布、恋爱/社交/职业等章节）
  - strategies/：自信独行型、人际掌控型、持续精进型、社交投入型（补充全球分布、恋爱表现、职业选择、成长方向等章节）
- **新建页面**（2个）：
  - theory/MBTI理论-角色概述.md — 角色分组总论（四种角色的划分逻辑与使用场景）
  - theory/MBTI理论-策略概述.md — 策略分组总论（心智×身份交叉形成四种策略）
- **wiki-index.md 更新**：
  - `[[overview]]` 改为 `[[MBTI理论-体系总览]]`
  - 体系总览部分新增角色概述、策略概述两条目
- **相关页面链接更新**：所有页面的父级链接从 `[[overview]]` 改为 `[[MBTI理论-体系总览]]`
